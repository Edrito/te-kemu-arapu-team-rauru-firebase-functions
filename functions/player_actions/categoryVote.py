...
def handle_action(data: dict, db: FirestoreClient):
    ...
    category_votes = gameState.get("categoryVotes") if gameState.get("categoryVotes") else {}
    category_votes[player_id] = category

    if len(category_votes.keys()) == len(participants):
    ...
    gameState["categoryVotes"] = category_votes
